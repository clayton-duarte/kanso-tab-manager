...
export async function loadSession(): Promise<SessionState> {
  try {
    const result = (await chrome.storage.local.get([SESSION_KEY])) as {
      [SESSION_KEY]?: Partial<SessionState>;
    };
...
export async function loadPortable(): Promise<PortableState> {
  try {
    const result = (await chrome.storage.local.get([PORTABLE_KEY])) as {
      [PORTABLE_KEY]?: Partial<PortableState>;
    };
 ...
 */
export async function savePortable(
  portable: Partial<PortableState>
): Promise<void> {
